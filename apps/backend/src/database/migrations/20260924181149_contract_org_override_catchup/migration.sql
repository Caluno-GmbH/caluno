...
-- on the old sentence — no optional line to switch on, and (for the oldest
-- snapshots) no postcode or town either.
--
...
--   * only rows still carrying an exact preset shape are touched, so a
...
-- post-rename form. Two preset shapes land here:
--   * pre-zip/city:  "Zwischen dem {orgName}, {orgStreet}, und"
--   * main's zip/city + trailing und (no parties-additional yet):
--     "Zwischen dem {orgName}, {orgStreet}, {orgZip} {orgCity} und"
-- Nothing upstream backfills the optional line into either shape — the preset
-- gained it; stored templates did not.

-- 1a. Oldest shape: add zip/city, move conjunction, insert optional line.
...
-- 1b. Intermediate shape already on main: zip/city present, trailing "und",
--     no optional line. Move conjunction and insert the line; do not re-add
--     zip/city fields that are already bound.
UPDATE "document_templates" dt
SET "body" = jsonb_set(
  dt."body",
  '{blocks}',
...
WHERE dt."kind" = 'CONTRACT'
  AND dt."body"::text LIKE '%"Zwischen dem {orgName}, {orgStreet}, {orgZip} {orgCity} und"%'
  AND NOT EXISTS (
    SELECT 1
    FROM jsonb_array_elements(dt."body"->'blocks') AS blk,
         jsonb_array_elements(coalesce(blk->'lines', '[]'::jsonb)) AS line
